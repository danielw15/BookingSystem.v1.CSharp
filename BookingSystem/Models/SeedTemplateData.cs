...
namespace BookingSystem.Models
{
    ...
    {
        public static void InitializeTemplate(IServiceProvider serviceProvider)
        {
            using (var context = new BookingSystemContext(serviceProvider.GetRequiredService<DbContextOptions<BookingSystemContext>>()))
            {
                // DB Table name instead of .Event
                if (context.Event.Any())
                {
                    return;
                }
                context.Event.AddRange(
                    //new Object, which object you wanna add to the table. Event in this example
                    new Event
                    {
                        UserId = 5,
